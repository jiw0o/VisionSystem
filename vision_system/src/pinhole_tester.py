...
class UNet :
    def __init__(self, version, input_shape=[224, 224, 1], class_n=2, verbose=False) :
        ...
        if class_n == 2 :
            self.model.compile(optimizer='adam', loss='binary_crossentropy', metrics=[fn, tn])
        elif class_n > 2 :
            self.model.compile(optimizer='adma', loss='categorical_crossentropy', metrics=[fn, tn])
...
class Pintrigger :
    def __init__(self, idx) :
        ...
        # Set Tester and Define Messages
        ...
        for i in range(1, sample+1) :
            ...
            for h in range(0, height):
                if img[h, w] == 255 and color == 0 :
                    #if self.idx == 2 : print("carbon", h-prev)
    ...
    def read_image(self, msg) :
        ...
        if t == 16 :
            ...
            #out = self.bridge.cv2_to_imgmsg(threshed, "mono8")
            #self.img_pub.publish(out)
            self.cnt += 1
            if self.cnt > 10 :
